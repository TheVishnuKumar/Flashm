...
**cacheable=true works quite differently as it will make server call if data has been stale for a few seconds. The timings are set by Salesforce for when to get data from Salesforce. But Flashm won't make server call in any condition because of cache doesn't refresh automatically.**
